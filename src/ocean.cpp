...
Ocean::Ocean(int numStones, int numPreys, int numPredators, int live_count, int time_reproduction, int satiety, int deathByStarvation) {
    ...
    std::vector<srt> v = getShuffle(numStones, numPreys, numPredators);
    
    for (int i = 0; i < numStones; i++) {
        cells[v[i].x][v[i].y].setObject(new Stone{&cells[v[i].x][v[i].y]});
        stuff.push_back(cells[v[i].x][v[i].y].getObject());

    }
    for (int i = numStones; i < numStones + numPreys; i++) {
        cells[v[i].x][v[i].y].setObject(new Prey{live_count, time_reproduction, &cells[v[i].x][v[i].y], ObjType::PREY});
        stuff.push_back(cells[v[i].x][v[i].y].getObject());
    }
...
void Ocean::print() const {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if(cells[i][j].getObject() && cells[i][j].getObject()->getType() == ObjType::STONE) {
                std::cout << STONE_N;
            } else if(cells[i][j].getObject() && cells[i][j].getObject()->getType() == ObjType::PREY) {
                std::cout << PREY_N;
            } else if(cells[i][j].getObject() && cells[i][j].getObject()->getType() == ObjType::PREDATOR) {
                std::cout << PREDATOR_N;
            } else {
                std::cout << NOTHING_N;
            }
