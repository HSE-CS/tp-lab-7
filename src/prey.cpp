...
Prey::Prey(int live_count, int time_reproduction, Cell * cell, ObjType type) : Object(cell, type) {
...
void Prey::live() {
    ...
    if (live_count-- > 0) {
        curr_time++;
        go();
        if ((live_count > 0) && (time_reproduction == curr_time) && (!cell->getOcean()->getCells()[x][y].getObject())) {
            cell->getOcean()->getCells()[x][y].setObject(new Prey{live_count+curr_time, time_reproduction, &(cell->getOcean()->getCells()[x][y]), ObjType::PREY});
